...
resize();
window.addEventListener('resize',()=>{resize();buildStars();buildNebula();});
...
function buildNebula(){
  const cols=[[245,197,24],[130,80,255],[40,120,255],[255,80,120]];
...
setInterval(()=>{
  ...
  shoots.push({
    ...
    vx:L?2+Math.random()*3:-(2+Math.random()*3),
...
document.addEventListener('click',e=>{
  for(let i=0;i<14;i++){
    ...
    sparks.push({x:e.clientX,y:e.clientY,vx:Math.cos(angle)*spd,vy:Math.sin(angle)*spd,r:1.5+Math.random()*2.5,life:1});
  }
});
...
(function draw(){
  ...
  stars.forEach(s=>{
    ...
    ctx.fillStyle=s.gold?`rgba(245,197,24,${s.a})`:`rgba(200,210,255,${s.a*.9})`;
    ctx.fill();
  });
  ...
  shoots.forEach((s,i)=>{
    ...
    s.tail.forEach((t,ti)=>{
      const r=ti/s.tail.length;
      ctx.beginPath();ctx.arc(t.x,t.y,s.size*r*.85,0,Math.PI*2);
      ctx.fillStyle=`rgba(245,215,80,${r*s.a*.4})`;ctx.fill();
    });
    ctx.beginPath();ctx.arc(s.x,s.y,s.size,0,Math.PI*2);
    ctx.fillStyle=`rgba(255,220,80,${s.a})`;
    ctx.shadowBlur=12;ctx.shadowColor='#f5c518';ctx.fill();ctx.shadowBlur=0;
    s.x+=s.vx;s.y+=s.vy;
    if(s.y>cv.height+50||s.x<-80||s.x>cv.width+80)shoots.splice(i,1);
  });

  sparks.forEach((p,i)=>{
    p.x+=p.vx;p.y+=p.vy;p.vy+=.07;p.life-=.035;
    if(p.life<=0){sparks.splice(i,1);return;}
    ctx.beginPath();ctx.arc(p.x,p.y,p.r,0,Math.PI*2);
    ctx.fillStyle=`rgba(245,197,24,${p.life})`;
    ctx.shadowBlur=8;ctx.shadowColor='#f5c518';ctx.fill();ctx.shadowBlur=0;
  });
...
/* SCROLL REVEAL — simple, no classes needed */
const allReveal = document.querySelectorAll(
  '#about, #skills, #projects, #contact, .sk, .pcard, .chip, .ctr, .clink'
);
